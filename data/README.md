...
```bash
python data/download_data.py            # both cohorts, ~0.9 GB, resumable
python data/download_data.py --check    # verify what is already present
```

Downloads are size-verified against the Content-Length GEO reports. The external
cohort arrives as a tar; untar it into `data/GSE164241/` before rebuilding.

The CZ CELLxGENE atlas cannot be fetched without a browser session -- download the
gingiva `.h5ad` files manually from the link in the table above.

## How to rebuild

One command runs the whole pipeline in the correct order:

```bash
python reproduce.py --download   # fetch raw data
python reproduce.py              # full pipeline
python reproduce.py --verify     # tests + audit only, no download needed
```

The individual stages, if you prefer to run them by hand:
...
`perio_task.json` (the marker-control task spec) is small and kept in this folder.
